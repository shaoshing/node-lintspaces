...
Validator.prototype.validate = function(path) {
	...
	return new Promise(function(resolve) {
		...
		stat(path)
			.then(this._fileExists.bind(this))
			.then(this._cleanUp.bind(this))
			.then(this._loadSettings.bind(this))
			.then(this._loadFile.bind(this))
			.then(this._loadIgnores.bind(this))
			.then(this._validateFile.bind(this))
			.then(this._validateLines.bind(this))
...
Validator.prototype._loadFile = function() {
	...
	return new Promise(function(resolve) {
		readFile(this._path, this._settings.encoding)
...
Validator.prototype._loadSettings = function() {
	return new Promise(function(resolve) {
		...
		// TODO: This can be an issue loading the current settings into a
		// reference of the validator instance when running the code async...
		...
		if (typeof this._settings.editorconfig === 'string') {
			...
			stat(this._settings.editorconfig)
				.then(function(stats) {
					if (!stats.isFile()) {
						this._fail(
...
/**
 * Validate if file to validate exists
 * @param stats is the stats response of fs.stat(path) call
 */
Validator.prototype._fileExists = function(stats) {
	if (!stats) {
		throw new Error('Missing "stats" parameter for _fileExists function');
	}

	if (!stats.isFile()) {
		this._fail(MESSAGES.PATH_ISNT_FILE.message.replace('{a}', this._path));
	}
};

/**
 * Validate rules based on complete file.
 */
Validator.prototype._validateFile = function() {
	this._validateNewlineMaximum();
	this._validateNewlinesEOF();
};

/**
 * Validate rules based on each line of file.
 */
Validator.prototype._validateLines = function() {
